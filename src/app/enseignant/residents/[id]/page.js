...
import { formatDate, getInitials, getResidentYear } from '@/lib/utils'
import { ACTIVITY_TYPE_LABELS, getCountForRequiredLevel } from '@/lib/logbook'
...
export default async function ResidentFichePage({ params }) {
  ...
  const [{ data: resident }, { data: realisations }, { data: objectives }, { data: travaux }, { data: progressRows }] = await Promise.all([
    ...
    admin.from('v_resident_niveau').select('procedure_id, count_expose, count_supervise, count_autonome, niveau_atteint').eq('resident_id', id),
  ])

  if (!resident) notFound()

  const year = getResidentYear(resident.residanat_start_date)
  const yearObjectives = (objectives ?? []).filter((objective) => objective.year === year && objective.required_level > 0)
  ...
  return (
    <div className="max-w-3xl p-5 md:p-8">
          ...
          <div className="space-y-1">
            {insufficient.map((objective) => (
              <p key={objective.procedure_id} className="text-xs text-orange-600">
                · {objective.procedure_id} - {objective.min_count} acte(s) requis au niveau attendu
              </p>
            ))}
          </div>
        </div>
        ...
        {(realisations ?? []).map((realisation) => (
          <div key={realisation.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-slate-500">{formatDate(realisation.performed_at)} · {ACTIVITY_TYPE_LABELS[realisation.activity_type] ?? '-'}</p>
            </div>
            <Badge status={realisation.status} />
          </div>
        ))}
        {(realisations ?? []).length === 0 && <p className="py-6 text-center text-sm text-slate-400">Aucun acte enregistré</p>}
      </div>
      ...
      <div className="space-y-2">
        ...
        {(travaux ?? []).length === 0 && <p className="py-6 text-center text-sm text-slate-400">Aucun travail scientifique enregistré</p>}
      </div>
    </div>
  )
}
