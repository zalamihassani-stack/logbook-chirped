...
const NAV = {
  ...
  resident: [
    ...
    { label: 'Referentiel', icon: Scissors, path: '/resident/referentiel' },
  ],
}
...
export default function AppLayout({ profile, children, badges = {} }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: '#E8F4FC' }}>
      <aside
        className="sticky top-0 hidden h-screen w-60 flex-shrink-0 flex-col md:flex"
      ...
      <header
        className="fixed left-0 right-0 top-0 z-40 flex h-12 items-center justify-between border-b px-4 md:hidden"
      ...
      <main className="flex-1 overflow-y-auto pb-20 pt-12 md:pb-0 md:pt-0">
        {profilePath && (
          <div className="sticky top-0 z-30 hidden justify-end px-5 pt-4 md:flex md:px-8">
            <Link
              href={profilePath}
              aria-label="Profil"
      ...
      {role === 'resident' && (
        <a
          ...
          className="fixed right-4 z-40 flex items-center gap-2 rounded-full px-4 py-2.5 shadow-lg md:hidden"
          style={{ bottom: '72px', backgroundColor: '#0D2B4E', color: 'white' }}
        >
          ...
          if (home) {
            return (
              <Link key={path} href={path} className="relative flex flex-1 flex-col items-center justify-center gap-0.5 py-1.5">
                <div
                  className=" -mt-5 flex h-12 w-12 items-center justify-center rounded-full border-2 shadow-lg"
                  style={{ backgroundColor: active ? '#7BB8E8' : '#2563eb', borderColor: '#0D2B4E' }}
                >
